<div class="animate__animated animate__fadeIn">
    {{-- 1. Header & Bilah Pencarian --}}
    <div class="row mb-4 align-items-center">
        <div class="col-md-6">
            <h4 class="fw-bold text-dark mb-1">Daftar Surat Keluar</h4>
            <p class="text-muted small mb-0">Kelola dan pantau seluruh arsip surat keluar STMC.</p>
        </div>
        <div class="col-md-6 mt-3 mt-md-0">
            <form action="{{ route('surat.keluar') }}" method="GET">
                <div class="input-group shadow-sm" style="border-radius: 10px; overflow: hidden;">
                    ...
                    @if (request('search'))
                        <a href="{{ route('surat.keluar') }}" class="btn btn-light border-0 d-flex align-items-center">
                            ...
                            <tr style="transition: all 0.2s ease;">
                                ...
                                <td class="text-center">
                                    <span class="text-muted" style="font-size: 0.85rem;">
                                        <i class="bi bi-calendar-event me-1"></i>{{ \Carbon\Carbon::parse($item->tanggal_surat)->format('d/m/Y') }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-{{ $color ?? 'info' }} bg-opacity-10 text-{{ $color ?? 'info' }} px-3 py-2 rounded-pill border border-{{ $color ?? 'info' }}-subtle" style="font-size: 0.7rem;">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center justify-content-center opacity-75">
                                        ...
                                        <p class="text-muted small mb-0">Belum ada data surat keluar yang tercatat di sistem.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
