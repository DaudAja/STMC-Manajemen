                        ...
                        <form action="{{ route('surat.update', $surat->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="alert alert-light border mb-4">
                                ...
                                <div class="row g-3">
                                    ...
                                    {{-- LOGIKA PERCABANGAN NOMOR SURAT --}}
                                    <div class="mb-3">
                                        <label for="nomor_surat" class="form-label fw-bold">
                                            Nomor Surat
                                            <span
                                                class="badge {{ $surat->category->jenis == 'keluar' ? 'bg-danger' : 'bg-success' }} ms-2">
                                                {{ ucfirst($surat->category->jenis) }}
                                            </span>
                                        </label>

                                        {{-- Input Field dengan Logika Readonly --}}
                                        <input type="text"
                                            class="form-control {{ $surat->category->jenis == 'keluar' ? 'bg-secondary bg-opacity-10' : '' }} @error('nomor_surat') is-invalid @enderror"
                                            id="nomor_surat" name="nomor_surat"
                                            value="{{ old('nomor_surat', $surat->nomor_surat) }}" {{-- INI KUNCINYA: Jika jenis 'keluar', tambahkan atribut readonly --}}
                                        ...
                                        {{-- Pesan Bantuan yang Berbeda --}}
                                        @if ($surat->category->jenis == 'keluar')
                                            <div class="form-text text-danger">
                                                <i class="bi bi-lock-fill"></i>
                                                Nomor ini digenerate otomatis oleh sistem STMC dan <b>tidak boleh diubah</b>
                                                demi menjaga urutan arsip.
                                            </div>
                                        @else
                                            <div class="form-text text-success">
                                                <i class="bi bi-pencil-fill"></i>
                                                Silakan ubah nomor ini jika tidak sesuai dengan nomor pada fisik surat yang
                                                diterima.
                                            </div>
                                        @endif
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control bg-light"
                                            value="{{ $surat->category->nama_kategori }}" readonly>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="form-text text-muted">Biarkan kosong jika tidak ingin mengubah file PDF saat
                                    ini.</div>
